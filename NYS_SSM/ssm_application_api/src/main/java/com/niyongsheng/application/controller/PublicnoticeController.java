 ...
 */
@RestController
@RequestMapping(value = "/publicnotice", produces = MediaType.APPLICATION_JSON)
@Api(value = "公告", produces = MediaType.APPLICATION_JSON)
@Validated
public class PublicnoticeController {
    ...
    @ResponseBody
    @RequestMapping(value = "/selectPublicnoticeList", method = RequestMethod.GET)
    @ApiOperation(value = "查询公告列表", notes = "参数描述", hidden = false)
    @ApiImplicitParams({
    ...
    public ResponseDto<Publicnotice> selectPublicnoticeList(HttpServletRequest request, Model model,
        ...
        // 1.是否分页，调用service的方法
        List<Publicnotice> list = null;
        if (isPageBreak) {
            try {
                // 2.1分页查询 设置页码和分页大小
                PageHelper.startPage(pageNum, pageSize, false);
                list = publicnoticeService.selectByFellowshipMultiTable(Integer.valueOf(fellowship));
            } catch (Exception e) {

...
@interface NYSEmitterViewController ()
@property(nonatomic, assign) EmitterAnimationType animation_type;
@end

@implementation NYSEmitterViewController

- (instancetype)initWithEmitterAnimationType:(EmitterAnimationType)EmitterAnimationType {
    if (self == nil) {
        self = [super init];
    }
    _animation_type = EmitterAnimationType;
    return self;
}
...
- (void)showAnimation {
    switch (_animation_type) {
        case EmitterAnimationColourbar:
            [self caiDai];
            break;
            
        case EmitterAnimationSnow:
            [self snow];
            break;
            
        case EmitterAnimationRain:
            [self rain];
            break;
            
        case EmitterAnimationFire:
            [self fire];
            break;
...
- (void)snow {
    ...
    // 粒子发射位置
    snowEmitter.emitterPosition = CGPointMake(NScreenWidth/2,-20);
    // 发射源的尺寸大小
...
- (void)rain {
    ...
    rainEmitter.emitterSize     = CGSizeMake(NScreenWidth*2, NScreenHeight);
...
- (void)caiDai {
    CAEmitterLayer *snowEmitter = [CAEmitterLayer layer];
    //例子发射位置
    snowEmitter.emitterPosition = CGPointMake(NScreenWidth/2,-30);
    snowEmitter.preservesDepth=YES;
    //    snowEmitter.lifetime=0.1;
    
    //发射源的尺寸大小
    snowEmitter.emitterSize = CGSizeMake(NScreenWidth*2, NScreenHeight);
    //发射模式
